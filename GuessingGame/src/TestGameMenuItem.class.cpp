...
TestGameMenuItem::TestGameMenuItem(GameState& gameState, Vector2D<uint_fast32_t> const pos, std::string const &sprite) : MenuItem(pos, sprite), _gameState(gameState) {
	return;
}

// Copy constructor
TestGameMenuItem::TestGameMenuItem(TestGameMenuItem const &src) : MenuItem(src), _gameState(src.getGameState()) {
	*this = src;
	return;
}

// Deconstructor
TestGameMenuItem::~TestGameMenuItem(void) {
	return;
}

// Overload equals operators for copy constructor
TestGameMenuItem		&TestGameMenuItem::operator=(TestGameMenuItem const &rhs) {
	if (this != &rhs) {}
	return *this;
}

// Getters --
GameState				&TestGameMenuItem::getGameState(void) const {
