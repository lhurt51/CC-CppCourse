...
BackGameMenuItem::BackGameMenuItem(GameState& gameState, Vector2D<uint_fast32_t> const pos, std::string const &sprite) : MenuItem(pos, sprite), _gameState(gameState) {
	return;
}

// Copy constructor
BackGameMenuItem::BackGameMenuItem(BackGameMenuItem const &src) : MenuItem(src), _gameState(src.getGameState()) {
	*this = src;
	return;
}

// Deconstructor
BackGameMenuItem::~BackGameMenuItem(void) {
	return;
}

// Overload equals operators for copy constructor
BackGameMenuItem		&BackGameMenuItem::operator=(BackGameMenuItem const &rhs) {
	if (this != &rhs) {}
	return *this;
}

// Getters --
GameState				&BackGameMenuItem::getGameState(void) const {
